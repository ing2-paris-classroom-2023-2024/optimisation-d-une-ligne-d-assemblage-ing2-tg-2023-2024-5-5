...
{
    visite[num] = composante;

    pArc arc = sommet[num]->arc;

    while (arc != NULL)
    {
        if (visite[arc->sommet] == -1)
            DFS(sommet, arc->sommet, visite, composante);
        arc = arc->arc_suivant;
    }
}
...
int main()
{
    ...
    char* visite = (char*)malloc(g->ordre * sizeof(char));
    int composante = 0;
    ...
    for (int i = 0; i < g->ordre; i++)
    {
        ...
        {
            composante++;
            DFS(g->pSommet, i, visite, composante);
        }
    }

    printf("\nComposantes connexes : \n");

    for (int i = 1; i <= composante; i++)
    {
